...
const Table = () => {
  ...
  const renderForm = () => {
    const renderModuleCheckboxes = (module) => {
      return (
        <Row className="mb-2">
          <Col xs={12}>
          ...
          <Col xs={12} className="d-flex justify-content-around">
            {["Enable", "Write", "Read", "Edit", "Delete"].map(
              (permission, index) => (
                  ...
                  <Input
                    ...
                    id={`${module.name
                      ...
                      .replace(
                        /\s+/g,
                        "-"
                      )}-${permission.toLowerCase()}-checkbox`}
                  />
                  <Label
      ...
      return (
        <Row tag={Form} onSubmit={handleSubmit(onSubmit)}>
          ...
          {modules.map((module, index) => (
            <Fragment key={index}>{renderModuleCheckboxes(module)}</Fragment>
          ))}
